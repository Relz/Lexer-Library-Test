...
TEST(or_token, determining_if_stay_alone)
{
	ExpectTokenInformations(
		"||", { TokenInformation(Token::OR, StreamString("||", StreamPosition())) });
}

TEST(or_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(
		" || ", { TokenInformation(Token::OR, StreamString("||", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		";||;",
...
TEST(or_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		"||;",
		{ TokenInformation(Token::OR, StreamString("||", StreamPosition())),
...
TEST(or_token, determining_if_stay_between_numbers)
{
	ExpectTokenInformations(
		"1||1",
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString("1.1", StreamPosition())),
		  TokenInformation(Token::OR, StreamString("||", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString("1_E+1", StreamPosition())),
		  TokenInformation(Token::OR, StreamString("||", StreamPosition(1, 6))),
...
TEST(or_token, not_determining_if_part_of_string_literal)
{
	ExpectTokenInformations(
...
TEST(or_token, not_determining_if_part_of_comment)
{
