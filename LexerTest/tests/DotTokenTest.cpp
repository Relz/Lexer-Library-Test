...
{
	ExpectTokenInformations(L".", { TokenInformation(Token::DOT, StreamString(L".", StreamPosition())) });
}

TEST(dot_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(L" . ", { TokenInformation(Token::DOT, StreamString(L".", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		L";.;",
		{ TokenInformation(Token::SEMICOLON, StreamString(L";", StreamPosition())),
		  TokenInformation(Token::DOT, StreamString(L".", StreamPosition(1, 2))),
...
TEST(dot_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		L".;",
		{ TokenInformation(Token::DOT, StreamString(L".", StreamPosition())),
...
TEST(dot_token, not_determining_if_stay_between_numbers)
{
	...
	ExpectTokenInformations(L"1.1.1", { TokenInformation(Token::UNKNOWN, StreamString(L"1.1.1", StreamPosition())) });
	ExpectTokenInformations(
