...
{
	ExpectTokenInformations(L"*", { TokenInformation(Token::MULTIPLY, StreamString(L"*", StreamPosition())) });
}

TEST(multiply_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(L" * ", { TokenInformation(Token::MULTIPLY, StreamString(L"*", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		L";*;",
		{ TokenInformation(Token::SEMICOLON, StreamString(L";", StreamPosition())),
		  TokenInformation(Token::MULTIPLY, StreamString(L"*", StreamPosition(1, 2))),
...
TEST(multiply_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		L"*;",
		{ TokenInformation(Token::MULTIPLY, StreamString(L"*", StreamPosition())),
...
TEST(multiply_token, determining_if_stay_between_numbers)
{
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString(L"1.1", StreamPosition())),
		  TokenInformation(Token::MULTIPLY, StreamString(L"*", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString(L"1_E+1", StreamPosition())),
		  TokenInformation(Token::MULTIPLY, StreamString(L"*", StreamPosition(1, 6))),
...
TEST(multiply_token, not_determining_if_part_of_plus_assignment)
{
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::MULTIPLY_ASSIGNMENT, StreamString(L"*=", StreamPosition())),
		  TokenInformation(Token::MULTIPLY, StreamString(L"*", StreamPosition(1, 3))) });
}
