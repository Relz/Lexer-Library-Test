...
TEST(less_or_equivalence_token, determining_if_stay_alone)
{
	ExpectTokenInformations(
		L"<=", { TokenInformation(Token::LESS_OR_EQUIVALENCE, StreamString(L"<=", StreamPosition())) });
}

TEST(less_or_equivalence_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(
		L" <= ", { TokenInformation(Token::LESS_OR_EQUIVALENCE, StreamString(L"<=", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		L";<=;",
		{ TokenInformation(Token::SEMICOLON, StreamString(L";", StreamPosition())),
		  TokenInformation(Token::LESS_OR_EQUIVALENCE, StreamString(L"<=", StreamPosition(1, 2))),
...
TEST(less_or_equivalence_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		L"<=;",
		{ TokenInformation(Token::LESS_OR_EQUIVALENCE, StreamString(L"<=", StreamPosition())),
...
TEST(less_or_equivalence_token, determining_if_stay_between_numbers)
{
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString(L"1.1", StreamPosition())),
		  TokenInformation(Token::LESS_OR_EQUIVALENCE, StreamString(L"<=", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString(L"1_E+1", StreamPosition())),
		  TokenInformation(Token::LESS_OR_EQUIVALENCE, StreamString(L"<=", StreamPosition(1, 6))),
