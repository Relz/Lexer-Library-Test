...
{
	ExpectTokenInformations(L"(", { TokenInformation(Token::LEFT_ROUND_BRACKET, StreamString(L"(", StreamPosition())) });
}

TEST(left_round_bracket_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(
		L" ( ", { TokenInformation(Token::LEFT_ROUND_BRACKET, StreamString(L"(", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		L";(;",
		{ TokenInformation(Token::SEMICOLON, StreamString(L";", StreamPosition())),
		  TokenInformation(Token::LEFT_ROUND_BRACKET, StreamString(L"(", StreamPosition(1, 2))),
...
TEST(left_round_bracket_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		L"(;",
		{ TokenInformation(Token::LEFT_ROUND_BRACKET, StreamString(L"(", StreamPosition())),
...
TEST(left_round_bracket_token, not_determining_if_stay_between_numbers)
{
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString(L"1.1", StreamPosition())),
		  TokenInformation(Token::LEFT_ROUND_BRACKET, StreamString(L"(", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString(L"1_E+1", StreamPosition())),
		  TokenInformation(Token::LEFT_ROUND_BRACKET, StreamString(L"(", StreamPosition(1, 6))),
