...
TEST(modulus_assignment_token, determining_if_stay_alone)
{
	ExpectTokenInformations(
		"%=", { TokenInformation(Token::MODULUS_ASSIGNMENT, StreamString("%=", StreamPosition())) });
}

TEST(modulus_assignment_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(
...
TEST(modulus_assignment_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		"%=;",
		{ TokenInformation(Token::MODULUS_ASSIGNMENT, StreamString("%=", StreamPosition())),
...
TEST(modulus_assignment_token, determining_if_stay_between_numbers)
{
	ExpectTokenInformations(
		"1%=1",
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString("1.1", StreamPosition())),
		  TokenInformation(Token::MODULUS_ASSIGNMENT, StreamString("%=", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString("1_E+1", StreamPosition())),
		  TokenInformation(Token::MODULUS_ASSIGNMENT, StreamString("%=", StreamPosition(1, 6))),
...
TEST(modulus_assignment_token, not_determining_if_part_of_string_literal)
{
	ExpectTokenInformations(
...
TEST(modulus_assignment_token, not_determining_if_part_of_comment)
{
