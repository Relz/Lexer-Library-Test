...
TEST(and_token, determining_if_stay_alone)
{
	ExpectTokenInformations(
		"&&", { TokenInformation(Token::AND, StreamString("&&", StreamPosition())) });
}

TEST(and_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(
		" && ", { TokenInformation(Token::AND, StreamString("&&", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		";&&;",
...
TEST(and_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		"&&;",
		{ TokenInformation(Token::AND, StreamString("&&", StreamPosition())),
...
TEST(and_token, determining_if_stay_between_numbers)
{
	ExpectTokenInformations(
		"1&&1",
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString("1.1", StreamPosition())),
		  TokenInformation(Token::AND, StreamString("&&", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString("1_E+1", StreamPosition())),
		  TokenInformation(Token::AND, StreamString("&&", StreamPosition(1, 6))),
...
TEST(and_token, not_determining_if_part_of_string_literal)
{
	ExpectTokenInformations(
...
TEST(and_token, not_determining_if_part_of_comment)
{
