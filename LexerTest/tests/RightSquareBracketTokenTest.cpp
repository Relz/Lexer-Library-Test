...
TEST(right_square_bracket_token, determining_if_stay_alone)
{
	ExpectTokenInformations(
		L"]", { TokenInformation(Token::RIGHT_SQUARE_BRACKET, StreamString(L"]", StreamPosition())) });
}

TEST(right_square_bracket_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(
		L" ] ", { TokenInformation(Token::RIGHT_SQUARE_BRACKET, StreamString(L"]", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		L";];",
		{ TokenInformation(Token::SEMICOLON, StreamString(L";", StreamPosition())),
		  TokenInformation(Token::RIGHT_SQUARE_BRACKET, StreamString(L"]", StreamPosition(1, 2))),
...
TEST(right_square_bracket_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		L"];",
		{ TokenInformation(Token::RIGHT_SQUARE_BRACKET, StreamString(L"]", StreamPosition())),
...
TEST(right_square_bracket_token, not_determining_if_stay_between_numbers)
{
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString(L"1.1", StreamPosition())),
		  TokenInformation(Token::RIGHT_SQUARE_BRACKET, StreamString(L"]", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString(L"1_E+1", StreamPosition())),
		  TokenInformation(Token::RIGHT_SQUARE_BRACKET, StreamString(L"]", StreamPosition(1, 6))),
